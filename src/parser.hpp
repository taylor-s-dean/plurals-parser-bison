  ...
  private:
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_multiplicative = 22,                   // multiplicative
        S_associative = 23,                      // associative
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
      ...
      {
        YY_ASSERT (tok == token::TOK_YYEOF || tok == token::TOK_YYerror || tok == token::TOK_YYUNDEF || tok == token::TOK_ASSIGN || tok == token::TOK_MOD || tok == token::TOK_THAN || tok == token::TOK_ELSE || tok == token::TOK_LT || tok == token::TOK_LE || tok == token::TOK_GT || tok == token::TOK_GE || tok == token::TOK_EQ || tok == token::TOK_NE || tok == token::TOK_AND || tok == token::TOK_OR || tok == token::TOK_LPAREN || tok == token::TOK_RPAREN);
      }
#else
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_THAN (location_type l)
      {
        return symbol_type (token::TOK_THAN, std::move (l));
      }
#else
      static
      symbol_type
      make_THAN (const location_type& l)
      {
        return symbol_type (token::TOK_THAN, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_NUMBER (uint v, location_type l)
      {
      ...
      static
      symbol_type
      make_NUMBER (const uint& v, const location_type& l)
      {
